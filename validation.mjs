...
async function suiteEdit() {
  ...
  try {
    ...
    try {
      ...
      // 2n) v11.51 CPF 利息（服务端 cron 加的 kind:'interest'）→ 进总额，在 CPF 明细里显示成「📈 利息」那行（有标签 chip）
      await pg.evaluate(() => { CPF = normCpf({ open: { oa: 1372.79, sa: 362.60, ma: 486.84 }, entries: [{ id: 's1', month: '2026-08', kind: 'salary', gross: 4400, oa: 1012, sa: 264, ma: 352, ts: 2 }, { id: 'i1', month: '2027-01', kind: 'interest', note: '2026 利息', oa: 48.05, sa: 18.13, ma: 24.34, ts: 1 }] }); CPF.on = true; buildCpfPage(); });
      const intShow = await pg.evaluate(() => { const b = cpfBal(); const ir = document.querySelector('#cpfBody .cpf-mrow-int'); return { total: Math.round((b.oa + b.sa + b.ma) * 100) / 100, rows: document.querySelectorAll('#cpfBody .cpf-mrow').length, intRow: !!ir, tag: !!(ir && ir.querySelector('.cpf-tag.t-int')), gain: !!(ir && ir.querySelector('.mtot.gain')), txt: ir ? ir.textContent.replace(/\s+/g, ' ').trim() : '' }; });
      ...
      await pg.setInputFiles('#cpfRcShotIn', { name: 's.png', mimeType: 'image/png', buffer: PNGBUF }); await pg.waitForTimeout(300);
      // 每格下面小号码 = 截图读到的值（对比用）：一样→绿，改动了→红；一键按钮走 app 蓝色（不是青绿）
      const cmp = await pg.evaluate(() => { const m = document.getElementById('cpfRc_ma'); m.value = '999'; m.dispatchEvent(new Event('input', { bubbles: true })); const oa = document.getElementById('cpfRcRead_oa'), ma = document.getElementById('cpfRcRead_ma'); const bs = getComputedStyle(document.getElementById('cpfRcUse')); return { oaTxt: oa.textContent.trim(), oaSame: /same/.test(oa.className), maDiff: /diff/.test(ma.className), maTxt: ma.textContent.trim(), btnGreen: bs.backgroundColor.replace(/\s/g, '').startsWith('rgb(18,185,129') || bs.color.replace(/\s/g, '').startsWith('rgb(18,185,129') }; });
      (/1,372.79/.test(cmp.oaTxt) && cmp.oaSame && cmp.maDiff && /你改动了/.test(cmp.maTxt) && !cmp.btnGreen) ? ok('CPF 截图：每格下面小号码=截图读到的值（一样绿、改动红「你改动了」）· 一键按钮走蓝色不是青绿', JSON.stringify({ oa: cmp.oaTxt })) : bad('CPF 截图对比号码/按钮色不对', JSON.stringify(cmp));
      await pg.evaluate(() => { const m = document.getElementById('cpfRc_ma'); m.value = '486.84'; m.dispatchEvent(new Event('input', { bubbles: true })); });
