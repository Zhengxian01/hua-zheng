...
async function suiteEdit() {
  ...
  try {
    ...
    try {
      ...
      // 2n) v11.51 CPF 利息（服务端 cron 加的 kind:'interest'）→ 直接进总额 + 收进利息卡历史，【不】混进每月供款明细
      ...
      const intShow = await pg.evaluate(() => { const b = cpfBal(); return { total: Math.round((b.oa + b.sa + b.ma) * 100) / 100, inList: !!document.querySelector('#cpfBody .cpf-mrow-int'), rows: document.querySelectorAll('#cpfBody .cpf-mrow').length, hist: !!document.querySelector('#cpfBody .cpf-intlist'), histTxt: (document.querySelector('#cpfBody .cpf-intlist') || {}).textContent || '' }; });
      (Math.abs(intShow.total - 3940.75) < 0.01 && !intShow.inList && intShow.rows === 1 && intShow.hist && /2026 利息/.test(intShow.histTxt) && /91/.test(intShow.histTxt)) ? ok('CPF 利息：进总额 3940.75 + 收进「历年利息」历史；每月供款明细只剩 1 行工资（不混进去）', JSON.stringify({ total: intShow.total, rows: intShow.rows })) : bad('CPF 利息混进明细/没进历史', JSON.stringify(intShow));
